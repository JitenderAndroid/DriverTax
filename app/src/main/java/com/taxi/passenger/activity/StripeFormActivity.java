	 ...
	 protected void onCreate(Bundle savedInstanceState)
		{
		  ...
		  txt_stripe_form =  findViewById(R.id.txt_stripe_form);
				...
				public void onClick(View v)
				  {
					 ...
					 Card card = new Card(number.getText().toString(), Integer.parseInt(expMonth.getText().toString()), Integer.parseInt(expYear.getText().toString()), cvc.getText().toString());
					 ...
					 if(validation)
					 {
						...
						new Stripe(StripeFormActivity.this, Constants.PUBLISHABLE_KEY).createToken(card, Constants.PUBLISHABLE_KEY, new TokenCallback()
						  {
					 ...
					 else if(!card.validateNumber())
					 {
						ProgressDialog.cancel();
						cusRotateLoading.stop();
						Common.showMkError(StripeFormActivity.this, getResources().getString(R.string.card_no_invaild));
					 }
					 else if(!card.validateExpiryDate())
					 {
						ProgressDialog.cancel();
						cusRotateLoading.stop();
						Common.showMkError(StripeFormActivity.this, getResources().getString(R.string.expiry_date_invalid));
					 }
					 else if(!card.validateCVC())
					 {
						ProgressDialog.cancel();
						cusRotateLoading.stop();
						Common.showMkError(StripeFormActivity.this, getResources().getString(R.string.cvc_code_error));
					 }
					 else
					 {
						ProgressDialog.cancel();
						cusRotateLoading.stop();
						Common.showMkError(StripeFormActivity.this, getResources().getString(R.string.Card_detail_error));
					 }
				  }
			 });
